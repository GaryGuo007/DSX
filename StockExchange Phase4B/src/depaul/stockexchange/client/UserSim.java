...
import depaul.stockexchange.driver.MainAutomatedTest;
...
public class UserSim implements Runnable {
    ...
    private Price makeRandomPrice(BookSide side, String product) { // This should match your format for storing a side - enum, String, etc
        ...
        try {
            p = PriceFactory.makeLimitPrice(String.format("%.2f", price));
        } catch(Exception ex) {
            System.out.printf("Failed to make price with :" + price + " from UserSim." + ex.getMessage());
        }
        return p;
    }
