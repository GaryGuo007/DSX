...
import depaul.stockexchange.publishers.PublisherBase;
import depaul.stockexchange.publishers.MarketDataDTO;
import depaul.stockexchange.publishers.InvalidPublisherDataException;
...
public class CurrentMarketPublisher extends PublisherBase {
    ...
    public synchronized void publishCurrentMarket(MarketDataDTO md) 
        ...
        if (md == null) {
            throw new InvalidPublisherDataException("The market data passed in "
                    + "can't be null.");
        }
        
        if (md.product == null || md.product.isEmpty()) {
            throw new InvalidPublisherDataException("The String stock symbol "
        ...
        if (md.buyVolume < 0) {
            throw new InvalidPublisherDataException("The BUY side volume taken "
                    + "from the MarketDataDTO passed in can't be negative.");
        }
        
        // Check if the SELL side volume is negative
        if (md.sellVolume < 0) {
            throw new InvalidPublisherDataException("The SELL side volume taken "
                    + "from the MarketDataDTO passed in can't be negative.");
        }
